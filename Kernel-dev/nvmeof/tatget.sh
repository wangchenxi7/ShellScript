...
Register_block_device () {
	...
	cd namespaces/10
	sudo echo ${block_device} > device_path
	echo 1 > enable
...
Start_memory_server () {
	...
	sudo echo "${target_ip}" > addr_traddr
	sudo echo rdma > addr_trtype
	sudo echo ${target_port} > addr_trsvcid
	sudo echo ipv4 > addr_adrfam
