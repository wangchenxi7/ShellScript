...
### Shell Scrip Control
running_times=1
tag="semeru-graphx-pagerank-${CPUMemPercentage}-mem"
...
do
  ...
  then
	...
	confVar="spark.app.name=GraphxPageRank"
  fi
  ...
  # run the application
	echo "spark-submit --class PageRankExample  --conf "${confVar}"  ${HOME}/jars/gpr.jar ~/data/${InputDataSet} ${AppIterations}"
  (time -p  spark-submit --class PageRankExample   --conf "${confVar}"  ${HOME}/jars/gpr.jar ~/data/${InputDataSet} ${AppIterations}) >> "${HOME}/Logs/${tag}.inputSet${InputDataSet}.Iteration${AppIterations}.heapSize${heapSize}.LocalMemPecentage${CPUMemPercentage}.${gcMode}.parallelGC${ParallelGCThread}.log" 2>&1
