...
do
  ...
  then
		...
		then	
			...
			#confVar="spark.executor.extraJavaOptions= -XX:+UseG1GC  -Xms${heapSize} ${youngGenRatio} ${initYoung} ${maxYoung}  -XX:ParallelGCThreads=${GCParallelism}  -XX:ConcGCThreads=2 
			confVar="spark.executor.extraJavaOptions= -XX:+UseG1GC  -Xms${heapSize} ${youngGenRatio} ${initYoung} ${maxYoung} -Xlog:gc=info "
		elif [ ${gcMode} = "STW" ]
		then
			## STW -  Parallel GC 
		...
		then
			...
			confVar="spark.executor.extraJavaOptions=  -Xms${heapSize} ${youngGenRatio} ${initYoung} ${maxYoung}  -XX:+UseConcMarkSweepGC  "
		elif	[ "${gcMode}" = "DEBUG_EXECUTOR"  ]
		then
			## STW, debug the executor in a step by step way
			confVar="spark.executor.extraJavaOptions= -agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=5005  -Xms${heapSize} ${youngGenRatio} ${initYoung} ${maxYoung}  -XX:ParallelGCThreads=${GCParallelism}  "
		else
			echo "!! GC Mode ERROR  !!"
			exit
		fi
  ...
  # run the application
  (time -p  spark-submit --class org.apache.spark.examples.SparkPageRank   --conf "${confVar}"  ${HOME}/Benchmark/SparkBench/target/scala-2.12/sparkapp_2.12-1.0.jar   /${mode}/${InputSet} ${Iter} ) >> "${mode}.inputSet${InputSet}.iter${Iter}.heapSize${heapSize}.${youngGenRatio}.${initYoung}.${maxYoung}.${gcMode}.${tag}.log" 2>&1 
