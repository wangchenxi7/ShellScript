...
input_data="${HOME}/DataSet/test.input"
...
# Do the execution
echo "time -p ${FLINK_HOME}/bin/flink run -p 16 ${HOME}/flink-1.10.1/examples/batch/LinearRegression.jar   --input ${input_data} --iterations ${iteration}  --output ${output_file}"
time -p ${FLINK_HOME}/bin/flink run -p 16 ${HOME}/flink-1.10.1/examples/batch/LinearRegression.jar --input ${input_data}  --iterations ${iteration}  --output ${output_file}
