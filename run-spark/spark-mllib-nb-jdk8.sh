...
do
  ...
  # run the application
  (time -p  spark-submit --class org.apache.spark.examples.mllib.NaiveBayesExample   --conf "${confVar}"  /mnt/ssd/wcx/Spark-app-jars/SparkApp-assembly-mllib-nb-${mode}.jar  /mllib/${InputSet}) >> "${AppName}.${mode}.inputSet${InputSet}.iter${Iter}.heapSize${heapSize}.${youngGenRatio}.${initYoung}.${maxYoung}.${gcMode}.${tag}.log" 2>&1 
