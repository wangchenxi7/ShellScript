...
if [ -z "$1" ]
then
  echo "Use default input set : out.web-NotreDame"
	InputSet="out.web-NotreDame"
else
  InputSet=$1
fi
...
if [ -z "$2" ]
then
	echo "Use defualt iteration number: 2"
	Iter="2"
else
  Iter=$2
fi
...
do
  ...
  # run the application
  (time -p  spark-submit --class org.apache.spark.examples.SparkTC   --conf "${confVar}"  /mnt/data/wcx/Spark-app-jars/SparkApp-assembly-tc-${mode}.jar   /basic/${InputSet}  ${Iter} ${partitionsNum} ) >> "${AppName}.${mode}.${InputSet}.iter${Iter}.heapSize${heapSize}.${youngGenRatio}.${initYoung}.${maxYoung}.${gcMode}.${tag}.log" 2>&1 
